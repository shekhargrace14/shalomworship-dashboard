...
//   }, [defaultKey])
    ...
    const activeKeys = ALL_KEYS.filter((k) =>
        normalizedSearch === "" ? true : k.toUpperCase() === normalizedSearch
    )
    ...
    return (
        <Dialog>
            ...
            <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto rounded-xl p-6">
                <DialogHeader className="space-y-1">
                    <DialogTitle className="text-xl font-bold tracking-tight">
                        Nashville Numbers Guide
                    </DialogTitle>
                    <DialogDescription className="text-sm text-muted-foreground">
                        A system where numbers replace absolute chord names, allowing clean transpositions into any key instantly.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="flex flex-col items-start justify-between gap-4 py-4 border-b border-muted">
                    <div className="relative w-full ">
                        ...
                        <Input
                            ...
                            placeholder="Filter by key (e.g., C, F#, Bb)"
                            className="pl-9 pr-8"
                        />
                        {searchKey && (
                            <Button
                    ...
                    <div className="flex gap-1">
                        ...
                        {ALL_KEYS.map((k) => (
                            <Button
                                key={k}
                                variant={normalizedSearch === k.toUpperCase() ? "default" : "outline"}
                                size="sm"
                                className="h-7 px-2.5 text-xs font-semibold rounded-md hover:bg-accent cursor-pointer"
                        ...
                        <table className="w-full text-left border-collapse text-sm">
                            <thead>
                                <tr className="bg-muted/60 border-b border-border font-medium text-muted-foreground">
                                    <th className="py-3 px-4 font-bold text-foreground bg-muted/80 sticky left-0 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] w-28 border-r">
                                        Nashville
                                    </th>
                                    {activeKeys.map((k) => (
                            ...
                            <tbody className="divide-y divide-border">
                                {NASHVILLE_ROWS.map((row) => (
                                    <tr key={row.degree} className="hover:bg-muted/30 transition-colors">
                                        ...
                                        <td className="py-3 px-4 font-mono font-bold text-foreground bg-card sticky left-0 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] border-r">
                                            {row.degree}
                                        </td>
                                        {activeKeys.map((k) => {
                                            ...
                                            return (
                                                <td key={k} className="py-3 px-4 text-center font-medium font-mono text-muted-foreground">
                                                    <span className={normalizedSearch !== "" ? "text-base font-bold text-primary" : ""}>
                                                        {chord}
                                                    </span>
                                                </td>
                                            )
                ...
                {activeKeys.length === 0 && (
                    <div className="text-center py-8 text-sm text-muted-foreground border border-dashed rounded-xl mt-4">
                        No matching key scale found. Type a valid musical scale note (A-G).
                    </div>
                )}
            </DialogContent>
        </Dialog>
