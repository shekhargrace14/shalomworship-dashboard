...
import { LanguageType, song, StatusType, TempoType, VersionType } from '@prisma/client';
...
export default function SongForm({ initialData }: Props) {
  ...
  const [creatorIds, setCreatorIds] = useState<Artist[]>([]);

  const [artistIds, setArtistIds] = useState<Artist[]>(initialData?.artists || []);
  ...
  const generateSlug = (title: string, creator: string) => {
    return slugify(`${title} ${creator}`, {
      lower: true,
      strict: true,
      trim: true,
    });
  ...
  useEffect(() => {
    ...
    setFormData((prev) => ({
      ...prev,
      slug: generateSlug(prev.title, creatorIds[0]?.title || ''),
    }));
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      creatorIds: creatorIds.map((artist) => artist.id),
    };
      ...
      <form onSubmit={handleSubmit}>
        <FieldGroup>
          ...
          <div className="flex justify-between items-end">
            <FieldSet>
              <FieldLegend>Add New Song</FieldLegend>

              <FieldDescription>Please fill all required fields.</FieldDescription>
            </FieldSet>
          ...
          {/* SEARCH VARIANTS */}
          <FieldSet>
            <FieldLegend>Search Variants</FieldLegend>
          ...
          {/* Artist */}
          <FieldSet>
            <FieldLegend>Artist Information</FieldLegend>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Field>
                ...
                <SelectArtist artists={artists} selectedArtists={creatorIds} setSelectedArtists={setCreatorIds} type="creator" />

                <FieldDescription>Select the creator/production/band/church of the song.</FieldDescription>
              </Field>
              ...
              <Field>
                ...
                <SelectArtist artists={artists} selectedArtists={artistIds} setSelectedArtists={setArtistIds} type="artist" />

                <FieldDescription>Select the artist(s) of the song</FieldDescription>
              </Field>
            </div>
          </FieldSet>
          ...
          {/* CONTENT */}
          <FieldSet>
            <FieldLegend>About Song</FieldLegend>

            <div className="space-y-4">
              <Field>
                <FieldLabel>About Song</FieldLabel>

                <Textarea name="about" rows={4} placeholder="Write about song..." value={formData.about} onChange={handleChange} />
              </Field>

              <Field>
                <FieldLabel>Excerpt</FieldLabel>

                <Textarea name="excerpt" rows={3} placeholder="Short excerpt..." value={formData.excerpt} onChange={handleChange} />
              </Field>
            </div>
          </FieldSet>
          ...
          {/* LANGUAGE */}
          <FieldSet>
            <FieldLegend>Language & Version</FieldLegend>
          ...
          <FieldSet>
            <div className="flex items-center gap-3">
              <Checkbox
                checked={formData.isChords}
                onCheckedChange={(checked) =>
                  setFormData((prev) => ({
                    ...prev,
                    isChords: Boolean(checked),
                  }))
                }
              />

              <FieldLegend>Chords</FieldLegend>
            </div>
            ...
            {formData.isChords && (
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-4">
                ...
                <Field>
                  ...
                  <Input name="bpm" placeholder="120" value={formData.bpm} onChange={handleChange} />
                </Field>
                ...
                <Field>
                  ...
                  <Input name="time" placeholder="4/4" value={formData.time} onChange={handleChange} />
                </Field>
          ...
          <FieldSet>
            ...
            <div className="space-y-4">
              {/* META DESCRIPTION */}
              <Textarea name="metaDescription" rows={4} placeholder="Meta description..." value={formData.metaDescription} onChange={handleChange} />
