...
export default function Lyrics({ initialData, isEdit }: any) {
  ...
  const removeArrangment = (id: number) => {
    setLyrics((prev) => ({
      ...prev,
      arrangement: prev.arrangement.filter((section) => section.id !== id),
    }));
  };
  const updateArrangement = (id: number, field: string, value: any) => {
    setLyrics((prev) => ({
      ...prev,
  ...
  const duplicateSection = (sectionId: number) => {
    setLyrics((prev) => ({
      ...
      arrangement: prev.arrangement.flatMap((section) => {
        ...
        return [section, duplicatedSection];
      }),
    }));
  };
  // Line
  ...
  const updateLine = (sectionId: number, lineId: number, field: 'english' | 'native' | 'translation', value: string) => {
    setLyrics((prev) => ({
      ...prev,
  ...
  const deleteLine = (sectionId: number, lineId: number) => {
    setLyrics((prev) => ({
      ...prev,
  ...
  const updateLineField = (sectionId: number, lineId: number, field: 'indent' | 'sectionBreak', value: number | boolean) => {
    setLyrics((prev) => ({
      ...prev,
  ...
  const duplicateLine = (sectionId: number, lineId: number) => {
    setLyrics((prev) => ({
      ...prev,
  ...
  const addChord = (sectionId: number, lineId: number) => {
    setLyrics((prev) => ({
      ...prev,
  ...
  const addGeneratedChord = (sectionId: number, lineId: number, chordData: any) => {
    setLyrics((prev) => ({
      ...prev,
  ...
  const updateChord = (sectionId: number, lineId: number, chordIndex: number, field: 'root' | 'number' | 'quality' | 'space', value: string) => {
    setLyrics((prev) => ({
      ...prev,
  ...
  const deleteChord = (sectionId: number, lineId: number, chordId: number) => {
    setLyrics((prev) => ({
      ...prev,
  ...
  // Move Chords
  const moveChordLeft = (sectionId: number, lineId: number, chordId: number) => {
    setLyrics((prev) => ({
      ...prev,
  ...
  const moveChordRight = (sectionId: number, lineId: number, chordId: number) => {
    setLyrics((prev) => ({
      ...prev,
      ...
      arrangement: prev.arrangement.map((section) => {
        ...
        return {
          ...
          lines: section.lines.map((line) => {
            ...
            if (index === -1 || index === chords.length - 1) return line;
  ...
  const moveSectionUp = (sectionId: number) => {
    setLyrics((prev) => {
      ...
      const index = arrangement.findIndex((section) => section.id === sectionId);

      if (index <= 0) return prev;

      [arrangement[index - 1], arrangement[index]] = [arrangement[index], arrangement[index - 1]];
  ...
  const moveSectionDown = (sectionId: number) => {
    setLyrics((prev) => {
      ...
      if (index === -1 || index === arrangement.length - 1) return prev;

      [arrangement[index], arrangement[index + 1]] = [arrangement[index + 1], arrangement[index]];
  ...
  const moveLineUp = (sectionId: number, lineId: number) => {
    setLyrics((prev) => ({
      ...prev,
  ...
  const moveLineDown = (sectionId: number, lineId: number) => {
    setLyrics((prev) => ({
      ...prev,
      ...
      arrangement: prev.arrangement.map((section) => {
        ...
        if (index === -1 || index === lines.length - 1) return section;
  ...
  return (
    <div className="p-4 rounded-3xl">
      ...
      <div className="flex gap-2 flex-col">
        <Button type="button" variant="outline" className="w-full cursor-pointer" onClick={addArrangement}>
          + Add Section
        </Button>

        <Button type="button" variant="default" className="w-full cursor-pointer" onClick={handleSubmit}>
          Save
        </Button>
      </div>
    </div>
